...
class TestEditMessageStreamingSafety:
    ...
    async def test_message_too_long_splits_into_continuations_not_silent_truncation(self):
        ...
        # 34000-char content well over the 32768 rich-message limit.
        oversized = "x" * 34000
    ...
    async def test_saturated_preview_dedups_repeat_oversized_edits(self):
        ...
        r1 = await adapter.edit_message("123", "456", "x" * 33000, finalize=False)
        ...
        # identically — no API calls. (34000 and 35000 chars both truncate to
        # the same "…(1/2)" preview; 66000 crosses into "(1/3)" — a real change
        # that SHOULD be delivered, at most one edit per ~32768 chars of growth
        # instead of one per 0.8s tick.)
        for grow in (34000, 35000):
        ...
        await adapter.edit_message("123", "456", "x" * 66000, finalize=False)
        assert adapter._bot.edit_message_text.await_count == 2
        # ...and saturates again at the new marker.
        await adapter.edit_message("123", "456", "x" * 66100, finalize=False)
        assert adapter._bot.edit_message_text.await_count == 2

        # A DIFFERENT oversized prefix (content changed within the first 32768)
        # must still go through.
        await adapter.edit_message("123", "456", "y" * 66100, finalize=False)
    ...
    async def test_saturated_preview_state_cleared_on_finalize(self):
        ...
        await adapter.edit_message("123", "456", "x" * 34000, finalize=False)
        assert ("123", "456") in adapter._last_overflow_preview

        result = await adapter.edit_message("123", "456", "x" * 34000, finalize=True)
        assert result.success is True
