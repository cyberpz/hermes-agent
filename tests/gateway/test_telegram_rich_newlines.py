...
class TestRichMessageTableProtection:
    ...
    def test_text_around_table_still_gets_hard_breaks(self, adapter):
        """Prose lines outside the table keep getting hard breaks; the table
        block itself is separated by blank lines (`_pad_gfm_tables`) so the
        rich renderer never merges it with the surrounding prose."""
        content = (
            "Intro line one\n"
            "Intro line two\n"
            "| H1 | H2 |\n"
        ...
        assert "| H1 | H2 |\n|----|----|\n| a | b |" in md
        # Table block is blank-line separated from the surrounding prose
        # (_pad_gfm_tables prevents the renderer from merging table + prose);
        # the prose newline before/after still gets the hard-break marker.
